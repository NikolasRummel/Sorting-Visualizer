...
const GraphVisualizerBFS: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (svgRef.current) {
            ...
            const nodesData: Node[] = Array.from({ length: numberOfNodes }, (_, index) => ({
                ...
                color: 'white', // Initialize all nodes as white for BFS color representation,
    ...
    const handlePause = async () => {
        ...
        if (isPaused) {
            await animateBFS(); // Start animation if paused
        }
    };
    ...
    const animateBFS = async () => {
        ...
        const sourceNodeId = 0; // Start BFS from the first node
    ...
    return (
        <div className="p-4 px-8 pl-12">
            ...
            <svg className="w-full h-[500px] mr-20 border border-gray-200 dark:border-neutral-800 p-2 bg-white dark:bg-black rounded-xl" ref={svgRef}>
                {nodes.map(node => (
                    <g key={node.id}>
                        <circle
                            cx={node.x}
...
export default GraphVisualizerBFS;
